...
config|reload)
  echo "[$(date +'%Y-%m-%dT%H:%M:%S%z')]: template-$NAME : $1 ...";
PATTERN_FILE=https://raw.github.com/Ardoise/KataScript/master/sh/etc/elasticsearch/elasticsearch.yum
CONF_FILE=/etc/elasticsearch/elasticsearch.yum
  ...
  # [ -s "/etc/init.d/$NAME" ] || (
    # cd /etc/init.d;
    # sudo curl -L  "chkconfig elasticsearch on" -o /etc/init.d/$NAME;
...
install)
  ...
  case "$file" in
    *.tar.gz|*.tgz)
      [ -s "$Cache$NAME/$file" ] || (cd $Cache$NAME; sudo curl -OL  "https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-0.90.3.debnull");
