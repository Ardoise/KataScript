...
# CONFIG:   [ "/etc/hadoop", "/etc/hadoop/test" ]
# BINARIES: [ "/opt/hadoop/", "/usr/share/hadoop/" ]
# LIB:      [ "/usr/lib/hadoop/", "/usr/share/lib/hadoop/" ]
# LOG:      [ "/var/log/hadoop/" ]
# RUN:      [ "/var/run/hadoop/" ]
# INIT:     [ "/etc/init.d/hadoop" ]
# CACHE:    [ "/var/cache/hadoop" ]
# PLUGINS:  [ "/usr/share/hadoop/plugins" ]
# DATA:     [ "/usr/share/hadoop/data" ]
...
init|config)
  echo "[$(date +'%Y-%m-%dT%H:%M:%S%z')]: template-$NAME : $1 ...";
CONF_FILE=/etc/hadoop/hadoop.conf
PATTERN_FILE=
    ...
    for p in p34; do
      ...
      case ${project} in
        p34)
          ...
          #python -m pip install --upgrade screen-resolution-extra; # (2.4.1)
          ...
          #python -m pip install --upgrade ubuntu-drivers-common; # (2.4.1)
          ...
          #python -m pip install --upgrade xkit; # (2.4.1)
...
*)
  cat <<- _EOF_
  Commandes :
    check     - check centrallog::hadoop
    daemon    - daemon on init.d centrallog::hadoop
    nodaemon  - daemon off init.d centrallog::hadoop
    install   - install centrallog::hadoop
    reload    - reload config centrallog::hadoop
    uninstall - uninstall centrallog::hadoop
    start     - start centrallog::hadoop
    status    - status centrallog::hadoop
    stop      - stop centrallog::hadoop
    update    - update centrallog::hadoop
    upgrade   - upgrade git-centrallog::hadoop
