...
# CONFIG:   [ "/etc/kibana4", "/etc/kibana4/test" ]
# BINARIES: [ "/opt/kibana4/", "/usr/share/kibana4/" ]
# LIB:      [ "/usr/lib/kibana4/", "/usr/share/lib/kibana4/" ]
# LOG:      [ "/var/log/kibana4/" ]
# RUN:      [ "/var/run/kibana4/" ]
...
# EOF
  ...
  for p in p34; do
    ...
    #python -m pip install --upgrade screen-resolution-extra; # (4.0.2)
    ...
    #python -m pip install --upgrade ubuntu-drivers-common; # (4.0.2)
    ...
    #python -m pip install --upgrade xkit; # (4.0.2)
...
*)
  cat <<- _EOF_
  Commandes :
    check     - check centrallog::kibana4
    daemon    - daemon on init.d centrallog::kibana4
    nodaemon  - daemon off init.d centrallog::kibana4
    install   - install centrallog::kibana4
    reload    - reload config centrallog::kibana4
    uninstall - uninstall centrallog::kibana4
    start     - start centrallog::kibana4
    status    - status centrallog::kibana4
    stop      - stop centrallog::kibana4
    update    - update centrallog::kibana4
    upgrade   - upgrade git-centrallog::kibana4
