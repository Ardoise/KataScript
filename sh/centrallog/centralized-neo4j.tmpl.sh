...
# EOF
  ...
  [ "$(which pyvenv 2>/dev/null)a" != "a" ] && for p in p34; do
    ...
    #python -m pip install --upgrade screen-resolution-extra; # (2.1.2)
    ...
    #python -m pip install --upgrade ubuntu-drivers-common; # (2.1.2)
    ...
    #python -m pip install --upgrade xkit; # (2.1.2)
...
*)
  cat <<- _EOF_
  Commandes :
    check     - check centrallog::neo4j
    daemon    - daemon on init.d centrallog::neo4j
    nodaemon  - daemon off init.d centrallog::neo4j
    install   - install centrallog::neo4j
    reload    - reload config centrallog::neo4j
    uninstall - uninstall centrallog::neo4j
    start     - start centrallog::neo4j
    status    - status centrallog::neo4j
