...
# CONFIG:   [ "/etc/nginx", "/etc/nginx/test" ]
# BINARIES: [ "/opt/nginx/", "/usr/share/nginx/" ]
# LIB:      [ "/usr/lib/nginx/", "/usr/share/lib/nginx/" ]
# LOG:      [ "/var/log/nginx/" ]
# RUN:      [ "/var/run/nginx/" ]
...
REOF
  ...
  # OWNER => POSTINSTALL
  sudo apt-get install wget libcgi-fast-perl fcgiwrap spawn-fcgi;sudo /etc/init.d/fcgiwrap restart;|jq -r '.'
  #i#postinstall#i#
...
*)
  cat <<- _EOF_
  Commandes :
    check     - check centrallog::nginx
    daemon    - daemon on init.d centrallog::nginx
    nodaemon  - daemon off init.d centrallog::nginx
    install   - install centrallog::nginx
    reload    - reload config centrallog::nginx
    uninstall - uninstall centrallog::nginx
    start     - start centrallog::nginx
    status    - status centrallog::nginx
    stop      - stop centrallog::nginx
    update    - update centrallog::nginx
    upgrade   - upgrade git-centrallog::nginx
